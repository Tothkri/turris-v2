...
public class GameWindow extends JPanel implements ActionListener {
    ...
    public void constructor(int width, int height) {
        ...
        timer = new Timer(fps, (ActionEvent ae) -> {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    for (int j = 0; j < towers.size(); j++) {
                        ...
                        if (simulationticks % tickPerAttack == 0) {
                            ArrayList<Unit> enemyUnitsNerby = model.enemyUnitsNearby(i, towers.get(j));
                            for (int k = 0; k < enemyUnitsNerby.size(); k++) {
                                if (enemyUnitsNerby.get(k).getHp()>towers.get(j).getPower()) {
                                    enemyUnitsNerby.get(k).setHp(enemyUnitsNerby.get(k).getHp()-towers.get(j).getPower());
                                } else {
                                    
                                    model.getPlayers()[(i+1)%2].deleteUnit(enemyUnitsNerby.get(k));
                                }
                            }
                        }
                    }
                ...
                for (int q = 0; q < 2; q++) {
                    ...
                    for (Unit u : model.getPlayers()[q].getUnits()) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            if (minDistance > model.wayDiff(q, wayString, u.getType())) {
                                bestway = model.getPlayers()[q].convertWay(wayString);
                                minDistance = model.wayDiff(q, wayString, u.getType());
            ...
            if (!model.isOver()) {
                ...
                if (newtime == 0) {
                    ...
                    timeAndRoundLabel.setText("Time left: " + (ticks + 1) / (1000 / fps) + " s, round: " + (model.getRound() + 1) / 2);
                    activePlayerLabel.setText("Active player: "
                            + model.getPlayers()[model.getActivePlayer()].getName());
                }
            ...
            for (int q = 0; q < 2; q++) {
                ...
                for (Unit u : model.getPlayers()[q].getUnits()) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        if (minDistance > model.wayDiff(q, wayString, u.getType())) {
                            ArrayList<Node> newWay = model.getPlayers()[q].convertWay(wayString);
                            minDistance = model.wayDiff(q, wayString, u.getType());
                            bestway = newWay;
                        }
                    }
